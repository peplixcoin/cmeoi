...
export default function DineOrderHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        // Fetch my orders
        const myOrdersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/${username}`);
            ...
            if (familyMembersResponse.ok) {
              ...
              const allFamilyOrders = await Promise.all(
                familyUsernames.map(async (familyUsername: string) => {
                  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/${familyUsername}`);
                  if (response.ok) {
                ...
                <div className="flex flex-wrap justify-between items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getStatusBadgeClass(order.order_status)}`}>
                      Status: {order.order_status === 'completed' ? 'Served' : order.order_status === 'approved' ? 'Preparing' : 'Pending'}
                    </span>
                  ...
                  <div className="flex items-center">
                    {order.order_status === 'completed' && (
                      <div className="flex items-center text-green-600">
                        ...
                        <span className="font-medium text-sm sm:text-base">Order served successfully</span>
                      </div>
                    )}
                    ...
                    {order.order_status === 'approved' && (
                      <div className="flex items-center text-blue-600">
                        <CheckCircle size={16} className="mr-1 sm:mr-2" />
                        <span className="font-medium text-sm sm:text-base">Order is being prepared</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-4 sm:p-6 text-white shadow-md mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                ...
                <p className="text-sm sm:text-base opacity-90">Track all your dine-in orders</p>
              </div>
