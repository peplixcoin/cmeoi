...
import { Star, FileText, Clock, Package, ShoppingBag, MessageSquare } from "lucide-react";
...
export default function OnlineOrderPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/online/orders/${username}/latest`
        );
    ...
    const fetchFeedback = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/online/feedback/${username}`
        );
    ...
    const eventSource = new EventSource(
      `${process.env.NEXT_PUBLIC_API_URL}/api/online/orders/${username}/stream`
    );
  ...
  const downloadReceipt = (order: Order) => {
    ...
    pdf.text(
      `Order Status: ${getOrderStatusLabel(order.order_status)}`,
      14,
      finalY + 30
    );
    pdf.text(
    ...
    pdf.text("Thank you for shopping with us!", 14, finalY + 60);
  ...
  const handleFeedbackSubmit = async (orderId: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/online/feedback/submit`,
        {
          method: "POST",
          headers: {
      ...
      const updatedFeedbackList: Feedback[] = await feedbackResponse.json();
    ...
    <div className="min-h-screen py-4 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-gray-50 text-left">
                        <th className="px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-gray-700 rounded-tl-lg w-1/2">
                          Item
                        </th>
                        <th className="px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-gray-700 text-center w-1/6">
                          Qty
                        </th>
                        <th className="px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-gray-700 text-right w-1/6">
                          Price
                        </th>
                        <th className="px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-gray-700 text-right rounded-tr-lg w-1/6">
                          Total
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {order.items.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-800 text-center">
                            {item.qty}
                          </td>
                          <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-800 text-right">
                            ₹{item.item_price.toFixed(2)}
                          </td>
                          <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-800 text-right font-medium">
                            ₹{item.total_price.toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    ...
                    <tfoot>
                      <tr className="bg-indigo-50">
                        <td
                          colSpan={3}
                          className="px-2 sm:px-4 py-2 text-right font-semibold text-xs sm:text-sm text-gray-700"
                        >
                          Total Amount:
                        </td>
                        <td className="px-2 sm:px-4 py-2 text-right font-bold text-xs sm:text-sm text-indigo-700">
        ...
        <div className="flex justify-center mt-8 sm:mt-12">
          <button
            onClick={() => router.push("/online/order-history")}
                  ...
                  <div className="flex gap-1 sm:gap-2 justify-center">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        ...
                        size={20}
                        className={`cursor-pointer transition-all ${
                          star <= ratings.hygiene
                            ? "text-green-500"
                            : "text-gray-300"
                        }`}
                        onClick={() =>
                          setRatings({ ...ratings, hygiene: star })
                        }
                  ...
                  <div className="flex gap-1 sm:gap-2 justify-center">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        ...
                        size={20}
                        className={`cursor-pointer transition-all ${
                          star <= ratings.behavior
                            ? "text-purple-500"
                            : "text-gray-300"
                        }`}
                        onClick={() =>
                          setRatings({ ...ratings, behavior: star })
                        }
