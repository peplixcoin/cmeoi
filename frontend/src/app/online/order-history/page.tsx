        ...
        if (userDetailsResponse.ok) {
          ...
          if (userDetails.role !== 'family') {
            ...
            if (familyMembersResponse.ok) {
              ...
              const familyUsernames = familyMembers.map((member: { username: string }) => member.username);
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 text-left">
                      <th className="py-2 px-6 text-xs font-medium text-gray-700 w-1/2">Item Name</th>
                      <th className="py-2 px-6 text-xs font-medium text-gray-700 text-center w-1/6">Quantity</th>
                      <th className="py-2 px-6 text-xs font-medium text-gray-700 text-right w-1/6">Price</th>
                      <th className="py-2 px-6 text-xs font-medium text-gray-700 text-right w-1/6">Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {order.items.map((item, index) => (
                      <tr key={item.item_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="py-2 px-6 text-sm text-gray-800">{item.item_name}</td>
                        <td className="py-2 px-6 text-sm text-center text-gray-800">{item.qty}</td>
                        <td className="py-2 px-6 text-sm text-right text-gray-800">₹{item.item_price.toFixed(2)}</td>
                        <td className="py-2 px-6 text-sm text-right font-medium text-gray-800">₹{item.total_price.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-gray-50 p-4 sm:p-6 border-t border-gray-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  <div className="flex items-center">
                    {order.order_status === 'completed' && (
                      <div className="flex items-center text-green-600">
                        ...
                        <span className="font-medium text-sm">Order completed successfully</span>
                      </div>
                    )}
                    {order.order_status === 'pending' && (
                      <div className="flex items-center text-yellow-600">
                        <Clock size={16} className="mr-2" />
                        <span className="font-medium text-sm">Order is being processed</span>
                      </div>
                    )}
                    {order.order_status === 'approved' && (
                      <div className="flex items-center text-blue-600">
                        <CheckCircle size={16} className="mr-2" />
                        <span className="font-medium text-sm">Order is approved and on the way</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-indigo-700">₹{order.total_amt.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen py-4 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-4 sm:p-6 text-white shadow-md mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <h2 className="text-lg font-bold mb-1">Your Order Timeline</h2>
                <p className="text-sm opacity-90">Track all your online orders</p>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2 inline-flex items-center">
        ...
        {!loading && !error && activeTab === 'family-orders' && userRole === 'family' && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center mb-6">
            <AlertCircle className="text-red-500 mr-3 w-6 h-6" />
            <p className="text-red-600 text-sm">You cannot see your family orders</p>
          </div>
        )}
