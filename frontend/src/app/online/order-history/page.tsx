...
import { ShoppingBag, Calendar, Clock, CheckCircle, AlertCircle, ArrowLeft, User } from "lucide-react";
...
export default function OnlineOrderHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        // Fetch my orders
        const myOrdersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/online/orders/${username}`);
        ...
        const userDetailsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/user/${username}`);
        if (userDetailsResponse.ok) {
          const userDetails = await userDetailsResponse.json();
          setUserRole(userDetails.role);

          // Fetch family orders only if user is not a family member
          if (userDetails.role !== 'family') {
            const familyMembersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/user/${username}/family`);
            if (familyMembersResponse.ok) {
              const familyMembers = await familyMembersResponse.json();
              const familyUsernames = familyMembers.map((member: any) => member.username);
              ...
              const allFamilyOrders = await Promise.all(
                familyUsernames.map(async (familyUsername: string) => {
                  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/online/orders/${familyUsername}`);
                  if (response.ok) {
      ...
      return (
        <div className="bg-white rounded-xl shadow-sm p-6 sm:p-10 text-center">
          ...
          <p className="text-gray-600 text-sm sm:text-base max-w-md mx-auto">
            {activeTab === 'my-orders' 
              ? "You haven't placed any online orders yet. Browse our menu to place your first order."
              : "Your family members haven't placed any online orders yet."}
          </p>
          <button 
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        {ordersToRender.map((order) => {
          ...
          return (
            <div key={order.order_id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 transition-all duration-200 hover:shadow-md">
              <div className="p-4 sm:p-6 border-b border-gray-100">
                <div className="flex flex-wrap justify-between items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getStatusBadgeClass(order.order_status)}`}>
                      Status: {order.order_status.charAt(0).toUpperCase() + order.order_status.slice(1)}
                    </span>
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getPaymentBadgeClass(order.payment_status)}`}>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 text-left">
                      <th className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm font-medium text-gray-700 w-1/2">Item Name</th>
                      <th className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm font-medium text-gray-700 text-center w-1/6">Quantity</th>
                      <th className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm font-medium text-gray-700 text-right w-1/6">Price</th>
                      <th className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm font-medium text-gray-700 text-right w-1/6">Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {order.items.map((item, index) => (
                      <tr key={item.item_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm text-gray-800">{item.item_name}</td>
                        <td className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm text-gray-800 text-center">{item.qty}</td>
                        <td className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm text-gray-800 text-right">₹{item.item_price.toFixed(2)}</td>
                        <td className="py-2 sm:py-3 px-3 sm:px-6 text-xs sm:text-sm font-medium text-gray-800 text-right">₹{item.total_price.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-gray-50 p-4 sm:p-6 border-t border-gray-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  <div className="flex items-center">
                    {order.order_status === 'completed' && (
                      <div className="flex items-center text-green-600">
                        ...
                        <span className="font-medium text-sm sm:text-base">Order completed successfully</span>
                      </div>
                    )}
                    ...
                    {order.order_status === 'pending' && (
                      <div className="flex items-center text-yellow-600">
                        ...
                        <span className="font-medium text-sm sm:text-base">Order is being processed</span>
                      </div>
                    )}
                    ...
                    {order.order_status === 'approved' && (
                      <div className="flex items-center text-blue-600">
                        <CheckCircle size={16} className="mr-1 sm:mr-2" />
                        <span className="font-medium text-sm sm:text-base">Order is approved and on the way</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg sm:text-xl font-bold text-indigo-700">₹{order.total_amt.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen py-4 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-4 sm:p-6 text-white shadow-md mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <h2 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2">Your Order Timeline</h2>
                <p className="text-sm sm:text-base opacity-90">Track all your online orders</p>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-lg px-3 sm:px-4 py-1.5 sm:py-2 inline-flex items-center">
                <ShoppingBag className="mr-1 sm:mr-2 w-4 h-4 sm:w-5 sm:h-5" />
                <span className="font-semibold text-sm sm:text-base">
                  {activeTab === 'my-orders' ? orders.length : familyOrders.length} Orders
                </span>
              </div>
            </div>
          </div>
        ...
        {!loading && !error && activeTab === 'family-orders' && userRole === 'family' && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center mb-6">
            <AlertCircle className="text-red-500 mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6" />
            <p className="text-red-600 text-sm sm:text-base">You cannot see your family orders</p>
          </div>
        )}
