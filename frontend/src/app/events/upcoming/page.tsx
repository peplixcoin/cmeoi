...
import { Calendar, Clock, MapPin, Users, ArrowRight, Tag, AlertCircle } from "lucide-react";
...
export default function EventPage() {
  ...
  const calculateAvailability = (event: Event) => {
    ...
    const percentage = Math.max(0, Math.min(100, ((event.capacity - registered) / event.capacity) * 100));
    return percentage;
  };
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div>
              ...
              <p className="text-indigo-100 max-w-xl">
                From workshops to social gatherings, we have something for everyone. 
        ...
        {events.length === 0 && !loading && !error && (
          <div className="bg-white rounded-xl shadow-sm p-10 text-center max-w-2xl mx-auto">
            ...
            <p className="text-gray-600 max-w-md mx-auto mb-6">
              There are no upcoming events at the moment. Please check back later for new announcements.
            </p>
            <Link href="/">
        ...
        {events.length > 0 && !loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {events.map((event) => {
              ...
              const availabilityColorClass = getAvailabilityColorClass(availabilityPercentage);
              const spotsRemaining = event.capacity - (event.registeredUsers?.length || 0);
              
              return (
                <div key={event.eventId} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-all duration-300">
                  {/* Event Image */}
